...
router.get('/api/shorts/filter', authenticateToken, (req, res) => {
    ...
    try {
        ...
        return res.status(400).json({ status: 'Invalid query format', status_code: 400 });
    }
    ...
    db.query(query, (err, results) => {
        if (err) {
            ...
            return res.status(500).json({ message: 'Error retrieving filtered shorts', status_code: 500 });
        }

        if (results.length === 0) {
            return res.status(400).json({ status: 'No short matches your search criteria', status_code: 400 });
        }
        ...
        results = results.map(short => ({
            ...
            contains_content: search.keyword ? short.content.includes(search.keyword) : false,
            contains_author: search.author ? short.author.includes(search.author) : false
        }));
